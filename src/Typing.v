  ...
  Module Typing (PC : PATCHECK).
    ...
    Definition ok_data (Gamma : ctx) (d : data) : Prop :=
      forall T cons,
        ...
        tname_in T Gamma /\
          forall K,
            K \in (cons : set con) -> exists d ty, con_in K d ty T Gamma
    .
    ...
    | KAll : forall Gamma k ty,
        ok_kind Gamma k ->
        Gamma & BindTVar k |= {0 ~> TyFVar (length Gamma)} ty ~:: KiType ->
    ...
    Lemma ok_ctx_ok :
      forall Gamma,
        ok_ctx Gamma ->
        ok Gamma.
    Proof. introv Hctx ; induction* Hctx. Qed.
    #[export]
     Hint Resolve ok_ctx_ok : mcore.
    ...
    Proof.
      introv Hctx Hbind.
      induction* Hctx ; try (binds_cases Hbind ; apply_empty* ok_type_weakening).
      { Case "CtxEmpty". false. apply* binds_empty_inv. }
    ...
    Lemma ok_term_lct :
      forall Gamma t T,
        Gamma |= t ~: T ->
        lct T.
    Proof with eauto using ok_ctx_ok_type, ok_type_lct.
      introv hasType.
      induction* hasType...
